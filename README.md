...
Лаунчер требует вход в серверный аккаунт, хранит только session token в Windows
Credential Manager и поддерживает обычный выход и отзыв всех сессий. Пароль на диск
не записывается. Перед запуском выдаётся одноразовый game ticket; встроенный Forge-мод
передаёт его во время login handshake, связывает с ником и удаляет локальный файл.
